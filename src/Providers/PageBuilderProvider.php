...
/**
 * Contributes a "Slider" component to the page-builder editor.
 *
 * Discovered automatically when both this package and the page-builder module are
 * installed (providers-finder). The slider is a data-bound iterating container,
 * like the built-in `repeat`: bind it to a data source and it renders the authored
 * child template once per item, each wrapped as a slide inside the .zkslide markup
 * (see PageBuilder/slider.php). The public side still needs the slider assets —
 * the host enables them with `\Model\Assets\Assets::enable('slider')`.
 */
	...
	public static function components(): array
	{
		return [
			'slider' => [
				...
				'template' => __DIR__ . '/../PageBuilder/slider.php',
			],
		];
	}
}
